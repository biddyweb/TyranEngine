...
yaml_parser_load_document(yaml_parser_t* parser, yaml_event_t* first_event)
{
	...
	if (!yaml_parser_load_node(parser, &event)) {
		return 0;
	}
...
yaml_parser_load_scalar(yaml_parser_t* parser, yaml_event_t* first_event)
{
	...
	if (!yaml_parser_register_anchor(parser, index,
									 first_event->data.scalar.anchor)) {
		return 0;
	}
...
yaml_parser_load_sequence(yaml_parser_t* parser, yaml_event_t* first_event)
{
	...
	if (!STACK_INIT(parser, items, INITIAL_STACK_SIZE)) {
		goto error;
	}
	...
	if (!yaml_parser_register_anchor(parser, index,
									 first_event->data.sequence_start.anchor)) {
		return 0;
	}
...
yaml_parser_load_mapping(yaml_parser_t* parser, yaml_event_t* first_event)
{
	...
	if (!STACK_INIT(parser, pairs, INITIAL_STACK_SIZE)) {
		goto error;
	}
	...
	if (!yaml_parser_register_anchor(parser, index,
									 first_event->data.mapping_start.anchor)) {
		return 0;
	}
