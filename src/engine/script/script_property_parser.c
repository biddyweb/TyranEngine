...
const nimbus_resource* parse_resource_reference(const tyran_value* value, nimbus_resource_type_id type_id)
{
...
static void value_to_property(nimbus_component* component, const nimbus_component_definition_property* property, const tyran_value* value, nimbus_property_reader* reader)
{
	...
	switch (property->type) {
		...
		case NIMBUS_COMPONENT_DEFINITION_REFERENCE_RESOURCE:
			*((const nimbus_resource**)data) = parse_resource_reference(value, property->reference_resource_type_id);
			break;
		default:
...
static void parse_property(nimbus_component* component, nimbus_property_reader* reader, tyran_symbol symbol, const tyran_value* value)
{
	...
	value_to_property(component, property, value, reader);
}
...
{
	parse_property(component, &self->property_reader, symbol, value);
}
