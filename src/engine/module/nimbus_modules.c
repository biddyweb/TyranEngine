...
static nimbus_module* add(nimbus_modules* self, const char* name, size_t octet_size, nimbus_module_init_function func,
...
nimbus_event_definition* nimbus_modules_add_event_struct(nimbus_modules* self, const char* name, size_t struct_size,
														 u8t event_type_id)
{
	nimbus_event_definition* definition = &self->event_definitions[self->event_definitions_count++];
