								...
								yaml_mark_t start_mark, yaml_char_t** name)
{
	...
	if (!STRING_INIT(parser, string, INITIAL_STRING_SIZE)) {
		goto error;
	}
							  ...
							  int literal)
{
	...
	if (!STRING_INIT(parser, leading_break, INITIAL_STRING_SIZE)) {
		goto error;
	}
	if (!STRING_INIT(parser, trailing_breaks, INITIAL_STRING_SIZE)) {
		goto error;
	}
	...
	while ((int)parser->mark.column == indent && !IS_Z(parser->buffer)) {
		...
		if (!literal && (*leading_break.start == '\n')
			...
			if (!JOIN(parser, string, leading_break)) {
				goto error;
			}
			CLEAR(parser, leading_break);
		}
							 ...
							 int single)
{
	...
	if (!STRING_INIT(parser, whitespaces, INITIAL_STRING_SIZE)) {
		goto error;
	}
	...
	while (1) {
		...
		if (leading_blanks) {
			...
			if (!JOIN(parser, string, whitespaces)) {
				goto error;
			}
			CLEAR(parser, whitespaces);
		}
